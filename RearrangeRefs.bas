...
Sub RearrangeRefs()
    ...
    Else
        '----------------------------Ref 615 ----------------------------
        ArrRef = Array(ArrConn100, ArrConn105, ArrConn110, ArrConn115, ArrConn120, ArrConn130)
    End If
    ...
    For i = LBound(ArrRef) To UBound(ArrRef)
        For j = LBound(ArrRef(i)) To UBound(ArrRef(i))
            ...
            For Each cell In myrange
                CellRow = cell.Row
                If cell.Value = "AA" And Right(cell(1, 2).Value, l) = k Then
                    NextRow = NextRow + 1
                     Set rng1 = cell
                     Set rng2 = Cells(NextRow, 1)
                    
                    'Swap values
                    temp1 = rng1.Offset(, 0).Resize(, 12).Value
                    temp2 = rng2.Offset(, 0).Resize(, 12).Value

                    rng1.Offset(, 0).Resize(, 12).Value = temp2
                    rng2.Offset(, 0).Resize(, 12).Value = temp1
